...
class ListingsController < ApplicationController
  ...
  # All queries only allow search of current listings, past listings only visible to their owners.
  def index
    @listings =
      if params[:search]
        Listing.with_attached_images.eager_load(interactions: :listing).search(params).order(:at_time).where(
          'listings.at_time > ?', Time.zone.now
      ...
      else # Load future listings by ascending date
        Listing.eager_load(:interactions).with_attached_images.order(:at_time).where('at_time > ?', Time.zone.now)
        # Highlighting of interacted with functions is causing more queries from the view. Revise?
      end
  end
